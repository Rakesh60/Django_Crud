...
def reciepies(request):
    if request.method == "POST":
        ...
        Reciepe.objects.create(
            reciepe_name=rec_name, reciepe_description=rec_desc, reciepe_image=rec_image)
    ...
    if request.GET.get('search'):
        queryset = queryset.filter(
            reciepe_name__icontains=request.GET.get('search'))
...
def register_handel(request):  # sourcery skip: last-if-guard
    if request.method == 'POST':
...
def get_students(request):
    ...
    if request.GET.get('search'):
        ...
        queryset = queryset.filter(
            ...
            Q(student_id__student_id__icontains=search) | Q(
                student_name__icontains=search) | Q(student_email__icontains=search) | Q(department__department__icontains=search)
        )
    # Pagination
